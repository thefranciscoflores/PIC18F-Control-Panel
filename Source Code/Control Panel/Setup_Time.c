...
void Do_Setup_Time(void)
{
    ...
    setup_second = bcd_2_dec(second);           // Convert Second 
    setup_minute = bcd_2_dec(minute);           // Convert Minute       
    setup_hour = bcd_2_dec(hour);               // Convert Hour
    setup_day = bcd_2_dec(day);                 // Convert Day
    setup_month = bcd_2_dec(month);             // Convert Month
    setup_year = bcd_2_dec(year);               // Convert Year
...
void Increase_Time()
{
            switch (Select_Field)               		// Increment the proper field
            {
                ...
                case 4:
                    ...
                    if(setup_month == 4 ||setup_month == 6 ||setup_month == 9 ||setup_month == 11)
                    {
                        setup_day++;
                        if (setup_day == 31) setup_day = 1;
                        //break;
                    }
                    else if(setup_month == 1 ||setup_month == 3 ||setup_month == 5 ||setup_month == 7||setup_month == 8 ||setup_month == 10 ||setup_month == 12)
                    {
                        setup_day++;
                        if (setup_day == 32) setup_day = 1;
                        //break;
                    }
                    else if (setup_month == 2)
                    {
                        setup_day++;
                        if (setup_day == 28) setup_day = 1;
                        //break;
                    }
                    break;
